	...
	script.Parent.MouseButton1Up:Connect(function()
		...
		local function esp()
			...
			local function create_esp(player)
				...
				esp.box.Color = new_color3(255,255,0);
				esp.box.Thickness = 0.8;
				esp.box.Filled = false;

				esp.tracer = new_drawing("Line", true);
				esp.tracer.Color = new_color3(255,255,0);
				esp.tracer.Thickness = 0.25;

				esp.name = new_drawing("Text", true);
				esp.name.Color = new_color3(255,255,0);
				esp.name.Size = 1;
				esp.name.Center = true;

				esp.distance = new_drawing("Text", true);
				esp.distance.Color = new_color3(255,255,0);
					...
					if character and player.Team ~= game.Players.LocalPlayer.Team then
						if player.Character.Humanoid.Health > 1 then
							...
							if visible then
								...
								local width, height = floor(45 * scale_factor), floor(70 * scale_factor);
								local x, y = floor(position.X), floor(position.Y);

								esp.box.Size = new_vector2(width, height);
								esp.box.Position = new_vector2(floor(x - width * 0.5), floor(y - height * 0.4));

								esp.tracer.From = new_vector2(floor(viewport_size.X * 0.5), floor(viewport_size.Y * 0.5));
								esp.tracer.To = new_vector2(x, floor(y + height * 0.1));

								esp.name.Text = player.Name;
								esp.name.Position = new_vector2(x, floor(y - height * 0.7 - esp.name.TextBounds.Y));

								esp.distance.Text = floor(position.Z) .. " studs";
								esp.distance.Position = new_vector2(x, floor(y + height * -0.8));
							end
